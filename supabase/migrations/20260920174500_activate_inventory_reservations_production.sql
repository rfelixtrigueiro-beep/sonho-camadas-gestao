...
begin
  ...
  select p.airtable_record_id,
         ...
         coalesce(r.quantidade,0)::integer,
         (p.estoque_reservado_manual + coalesce(r.quantidade,0))::integer,
         greatest(
           greatest(coalesce(p.estoque,0),0) - p.estoque_reservado_manual -
           coalesce(r.quantidade,0),
           0
         )::integer
...
begin
  ...
  return jsonb_build_object('reserva_liberada',v_released,'estoque_devolvido',0,'status','cancelado');
end;
$$;
