...
export default function Registries({onCatalogChanged}:{onCatalogChanged:()=>void}){
 ...
 return <section className="registries-module" aria-label="Cadastros da Farm">
  <div className="registry-overview" aria-label="Resumo dos cadastros">{tabs.map(([id,label,Icon,total])=><article key={id}><span><Icon size={20}/></span><div><small>{label}</small><strong>{loading?'—':total}</strong><em>{loading?'Carregando':`${id==='supplies'?supplies.filter(item=>item.ativo).length:id==='printers'?printers.filter(item=>item.ativo).length:sellers.filter(item=>item.ativo).length} ativos`}</em></div></article>)}</div>
