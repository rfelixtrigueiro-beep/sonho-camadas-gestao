...
type CatalogPrinter={id:string;nome:string;potencia_watts:number;tarifa_energia_kwh:number;custo_maquina_hora:number};
...
export default function ProductSheet({userId}:{userId:string}){
 ...
 useEffect(()=>{let live=true;void (async()=>{
  ...
  try{
   const [sheetResult,suppliesResult,printersResult]=await Promise.all([
    ...
    supabase.from('farm_printers').select('id,nome,potencia_watts,tarifa_energia_kwh,custo_maquina_hora').eq('ativo',true).order('nome'),
   ]);
   if(sheetResult.error)throw sheetResult.error;
   if(live){
    setCatalog(suppliesResult.error?[]:(suppliesResult.data||[]) as CatalogSupply[]);
    setPrinters(printersResult.error?[]:(printersResult.data||[]) as CatalogPrinter[]);
   }
 ...
 async function createPrinter(event:React.SyntheticEvent<HTMLFormElement>){
  ...
  const {data,error}=await supabase.from('farm_printers').insert({nome:printerForm.nome.trim(),potencia_watts:number(printerForm.potencia_watts),tarifa_energia_kwh:number(printerForm.tarifa_energia_kwh),custo_maquina_hora:number(printerForm.custo_maquina_hora)}).select('id,nome,potencia_watts,tarifa_energia_kwh,custo_maquina_hora').single();
  ...
  else{setPrinters(current=>[...current,data as CatalogPrinter].sort((a,b)=>a.nome.localeCompare(b.nome,'pt-BR')));setPrinterForm(emptyPrinter);setPrinterRegistryOpen(false);setStatus('Impressora cadastrada e disponível na calculadora.')}
