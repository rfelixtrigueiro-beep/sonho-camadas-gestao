...
type Kit={id:string;nome:string;desconto_percentual:number;subtotal:number;preco_venda:number;exibir_portfolio:boolean;items:KitItem[]};
...
export default function Kits({administrator,userId,products}:{administrator:boolean;userId:string;products:KitSourceProduct[]}){
 ...
 const [name,setName]=useState(''),[discount,setDiscount]=useState('0'),[published,setPublished]=useState(false),[quantities,setQuantities]=useState<Record<string,number>>({});
 ...
 const load=useCallback(async()=>{
  setLoading(true);
  let request=supabase.from('farm_product_kits').select('id,nome,desconto_percentual,subtotal,preco_venda,exibir_portfolio').eq('ambiente','desenvolvimento').eq('ativo',true).order('criado_em',{ascending:false});
 ...
 function clearForm(){setName('');setDiscount('0');setPublished(false);setQuantities({});setFormOpen(false)}
 ...
 async function createKit(event:{preventDefault():void}){
  ...
  setSaving(true);
  const {data,error}=await supabase.from('farm_product_kits').insert({nome:name.trim(),categoria:'Kit',desconto_percentual:discountNumber,subtotal,preco_venda:finalPrice,exibir_portfolio:published,ativo:true,ambiente:'desenvolvimento',criado_por:userId}).select('id').single();
  if(error||!data){setMessage('Não foi possível salvar o kit. Tente novamente.');setSaving(false);return}
  const kitId=(data as {id:string}).id;
  ...
  if(itemsResult.error){await supabase.from('farm_product_kits').delete().eq('id',kitId);setMessage('Não foi possível salvar os itens do kit. Tente novamente.');setSaving(false);return}
  clearForm();setMessage(published?'Kit criado e publicado no ambiente de desenvolvimento.':'Kit criado como oculto no ambiente de desenvolvimento.');await load();setSaving(false);
 }
